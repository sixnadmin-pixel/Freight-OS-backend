...
from modules.rates.rate_types import VesselByVesselRateNew, VesselByVesselRatePatch, FAKRatesNew, FAKRatesPatch, SpecialRateNew, SpecialRatePatch, SurchargeNew, SurchargePatch
        ...
        try:
            async with pool.connection() as conn:
                async with conn.cursor(row_factory=dict_row) as cur:
                    ...
                    if surcharges:
                        for s in surcharges:
                            s_data = {**s.model_dump(), "rate_id": sr_id}
            ...
            async with pool.connection() as conn:
                async with conn.cursor(row_factory=dict_row) as cur:
                    if changes:
                        ...
                        await cur.execute(
                            "SELECT * FROM vessel_by_vessel_rate WHERE srid = %s", (srid,)
                        )
                        row = await cur.fetchone()
        ...
        try:
            async with pool.connection() as conn:
                async with conn.cursor(row_factory=dict_row) as cur:
                    ...
                    if surcharges:
                        for s in surcharges:
                            s_data = {**s.model_dump(), "rate_id": fak_id}
                            await cur.execute(build_insert("surcharge", s_data), s_data)
            ...
            async with pool.connection() as conn:
                async with conn.cursor(row_factory=dict_row) as cur:
                    if changes:
                        ...
                        await cur.execute(
                            "SELECT * FROM fak_rates WHERE fak_id = %s", (fak_id,)
                        )
                        row = await cur.fetchone()
        ...
        try:
            async with pool.connection() as conn:
                async with conn.cursor(row_factory=dict_row) as cur:
                    ...
                    if surcharges:
                        for s in surcharges:
                            s_data = {**s.model_dump(), "rate_id": sprid}
                            await cur.execute(build_insert("surcharge", s_data), s_data)
            ...
            async with pool.connection() as conn:
                async with conn.cursor(row_factory=dict_row) as cur:
                    if changes:
                        ...
                        await cur.execute(
                            "SELECT * FROM special_rate WHERE sprid = %s", (sprid,)
                        )
                        row = await cur.fetchone()
